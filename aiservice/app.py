...
def prepare_yolo_model():
    ...
    ### 3-1. final_flag=='true' : 모든 고양이 등록 완료라는 의미 ###
    if final_flag=='true':
        ### 4. [1번/2-1번] 사용자 업로드 이미지 bbox 예측 ###
        ...
        ### 5-1. [2-2번] Azure Blob Storage에 your_user_id에 해당되는 학습데이터셋 다운로드 ### #(테스트 2개씩: 기쁨/흰둥, 은애/이안)
        ...
        ### 5-2. [2-1번+2-2번] 학습데이터셋 구성 : 원본 + 데이터 증강 (로컬 누적) ### #(테스트 필요)
        print("[2-1번+2-2번] YOLO모델(2번)을 위한 학습데이터셋 구성")
        print("="*100)
        yaml_path, train_data_cnt, valid_data_cnt = config_train_datasets(base_url=BASE_URL,
                                                                          user_id=user_id, 
                                                                          name_and_bbox=name_and_bbox, 
                                                                          ai_datasets_base_path=os.path.join(BASE_URL,f'tmp/datasets/{user_id}'))
        print(f"Train, Valid = {train_data_cnt}, {valid_data_cnt}")
        print(yaml_path)
        print()

        ### 5-3. [2-3번] Azure Blob Storage에 학습데이터셋 덮어쓰기 (blob storage 누적) ###
        ### 6. [2-4번] corrupt JPEG 복구 및 yolo모델(2번) 학습 ###
        ### 7. [3번] YOLO모델(2번) 베스트 모델을 Azure Blob Storage에 저장 ###
        return jsonify(status_code=200, message=f"{user_id} : 학습데이터셋 구성 및 YOLO모델(2번) 준비 완료")
    ### 3-2. final_flag=='false' : 아직 고양이 등록 중이라는 의미 (로컬에 이미지를 저장시켜놓는다.) ###
    else:
